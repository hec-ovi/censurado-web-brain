# Finalize: lift the article into the publish payload
...
- **title**: the most compact, informative headline the story can honestly carry, drawn from
  ...
  mayor, minister); otherwise keep the person out of the title and let the body name them. It renders on one line on the portada, so
...
- **topics**: at most {{TOPIC_CAP}} tags, the most specific and optimized for the piece: the
  THEMES plus the named ENTITIES it is about (people, organizations, places), lowercase and
  accented where the language requires. A few sharp tags beat a cap full of loose ones.
- **keywords**: the narrower search terms specific to THIS article (the named people,
  places, organizations, and the distinctive concepts), drawn only from the article's text.
- **slug**: optional, lowercase words joined by hyphens; omit it to let the server derive it.
