...
- `mcp/` is the same surface as an MCP server (stdlib only, no install): 33 tools over the CLI verbs, so an agent that cannot run shell commands can still operate the portal. See [mcp/README.md](mcp/README.md).
...
Any stdio MCP client works. It exposes 33 tools over the same CLI verbs (articles, front-page
...
automation/            auto-batch.sh (one unattended batch) + supervisor/ (the 24/7 serve loop + REQUIREMENTS.md)
                       ...
                       + pipeline/ (durable article pipeline on DBOS: stateless api/cli steps, editorial gate, idempotent publish)
