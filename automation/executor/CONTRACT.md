...
- **The automation settings** (backend singleton `GET /automation-settings?include_secrets=true` — the executor is the one consumer that reads the raw `lanes.*.api_key`; the backend serves it redacted to everyone else — edited in the panel's Models section): `{lanes: {local: {base_url?, model?}, openrouter: {base_url?, model?}}, stages: {<node>: {lane?: "local"|"openrouter", model?}}}`. Before each firing the executor merges them over the file config (`derive.py`): the `local` lane overrides the `api` adapter, `openrouter` overrides (or creates, with `OPENROUTER_API_KEY` defaults) the remote adapter, and a stage entry re-points its node's adapter and model — a lane switch without its own model clears the node's file override so the lane default applies. The derived config is written beside the base one (`.executor.config.json`, a runtime artifact) so relative paths keep resolving; empty settings or an unreadable backend fall back to the file untouched.
...
- Each tick heartbeats `PUT /automation-status` with `{at, llama_ok, running, queued, effective}` — the executor's clock, the EFFECTIVE local lane's `/health` probe, the run in flight, the queue, and the effective model configuration (file + settings merged: each lane's endpoint/model, `key_set` but never the key, and the lane+model each stage resolves to). The panel edits those effective values; the poll loop keeps ticking while the worker runs a batch, so the heartbeat stays fresh mid-firing.
- A schedule's `prompt` (the panel's custom-run directive) rides into the batch as `--directive`; an empty prompt is a generic whole-newsroom run.
- **Startup catch-up**: at boot the executor runs what is PENDING — each enabled schedule's most recent due minute (last 24h) that never reached an outcome, whether missed while down or left as a stale `queued`/`running` record by a crash (the batch resumes idempotently under the same run id). Pending firings with the SAME setup (mode + prompt + authors) collapse into one batch: the first fires, the others' strips record `cubierta por <run_id>`. Older misses stay skipped.
