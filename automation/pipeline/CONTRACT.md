...
- **Batch**: `run.py batch --config <c> [--run-id <b>] [--mode preview|auto] [--authors a,b] [--directive "<texto>"]` runs the daily edition (config block `batch`): every author with a beat and attached sources pitches candidates (title + description + source link, count decided by the day) from their own feeds, the jefe de redaccion selects the edition (portada ranks). An operator `--directive` (a custom schedule's prompt) reaches both the candidates and the jefe prompts as `{directiva}` and steers them; empty means a free edition. The selected articles run the ordinary durable workflow in parallel (`batch.concurrency`, preview mode) with run ids `<batch>-n<i>`. Each article subprocess is bounded by `batch.article_timeout_s` (default 1800): an article past it (or one that cannot run) is killed and reported `failed` in its `batch-article` event and in `result.json`, and the batch continues with the rest; only a batch where EVERY article fails exits 4. Publishing is sequential in portada order (rank 1 last, so it tops the portada): automatic in `auto`, or later via `run.py batch-approve --config <c> --run-id <b>`. The selection persists in `<run_dir>/<batch>/plan.json` (re-running the same batch id resumes it; completed articles replay idempotently) and the outcome in `result.json`, schema [schema/batch-result.schema.json](schema/batch-result.schema.json). Each selected candidate's source link lands in the author's used-URL registry (`metadata.used_urls`, newest first, `batch.used_urls_cap` FIFO): the feeds context omits already-used titulars, so the same news is never pitched twice. The batch's own two calls (each author's pitch and the jefe's selection) run on `batch.adapter`, a named api-kind adapter defaulting to `api`, so an edition can be decided on the same lane its articles are written on.
- **Source photograph**: `source_images.enabled` adds one image-selection call after writing. `--source-url` supplies the main source (the batch forwards its candidate URL); research `read_urls` supply other candidates. The selector extracts photographs from those pages and uploads the selected one through `POST /media`. The final draft's API adapter/model also selects the image unless `source_images.adapter` names another API adapter. `vision: false` uses source titles, captions, and alt text; `vision: true` also sends the image pixels. Missing, unrelated, or unavailable photographs produce a text card. Contract: [source images](source_images/CONTRACT.md).
- **Image-only check**: `run.py image --config <c> --piece-file <article.json> --source-url <url>` uses that same selector and prints its image metadata or `{}`. It uploads media but does not publish an article. Exit 2 means invalid config/article input.
...
- **Secrets via env**: `backend.token_env` names the env var holding the operator token; an api-kind adapter takes `api_key_env` (the env var name) or a direct `api_key` (the executor's derived config carries the panel-stored key this way; a direct key wins over the env one).
...
- The DBOS system db at `<run_dir>/<run_id>/.dbos.sqlite`, one per run (each run.py process is its own executor; a per-run db keeps launch-time recovery scoped to that run, and resuming the same run id reattaches to it). Deleting it forgets that run's history.
...
Change `src/` freely; keep the schemas describing what the code actually reads and writes, keep this file matching them, and keep `tests/` passing (`.venv/bin/pytest automation/pipeline/tests automation/pipeline/source_images/tests`). Additive config/result fields: optional in the schema plus a minor `contractVersion` bump. Breaking shape changes: new schema file alongside the old, never edited in place.
